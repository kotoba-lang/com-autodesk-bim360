(ns autodesk_bim360.main
  "Kotodama WASM entrypoint for the Autodesk_bim360 clean-room actor (L4) — Clojure port.
  ...
  facts (`autodesk_bim360.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "autodesk_bim360")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"autodesk_bim360.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "autodesk_bim360-compat" :tier tier :entities entities} 200])
...
(defn start! [] :autodesk_bim360-compat/ready)
